...
void FrontCorners::Initialize() {
	...
	kFrontSetpoint = Robot::autoPref->GetDouble("FrontAngle", -29.75);

}
// Called repeatedly when this Command is scheduled to run
void FrontCorners::Execute() {
        // BEGIN AUTOGENERATED CODE, SOURCE=ROBOTBUILDER ID=EXECUTE
	Robot::arm->controller->SetSetpoint(kFrontSetpoint);
...
bool FrontCorners::IsFinished() {
	...
	return ((Robot::arm->armEncoder->GetDistance() - (kFrontSetpoint)) <= 0.25 &&
			(Robot::arm->armEncoder->GetDistance() - (kFrontSetpoint)) >= -0.25);    // END AUTOGENERATED CODE, SOURCE=ROBOTBUILDER ID=ISFINISHED
}
